...
int insertBefore(SList& sl, SNode* q, SNode* p) {
    if (q == NULL || p == NULL || sl.Head == NULL) 
        return 0; // Trả về 0 nếu danh sách trống hoặc node NULL
    
    if (sl.Head == q) { // Nếu q là Head, chèn p vào đầu danh sách
       insertHead(sl,p);
    }

    // Duyệt danh sách để tìm node trước q
    for (SNode* k = sl.Head; k->Next != NULL; k = k->Next) {
        if (k->Next == q) { // Tìm thấy node trước q
            k->Next = p;
            p->Next = q;
            return 1;
        }
...
void createSList_FromTextFile(SList& sl, char inputFileName[])
{
    ...
    int n;
    if (fscanf(fi, "%d", &n) != 1) // Kiểm tra lỗi đọc file
    {
        printf("Lỗi đọc số lượng phần tử từ file.\n");
        fclose(fi);
        return;
    }

    // Duyệt qua n phần tử và thêm vào danh sách liên kết
    for (int i = 0; i < n; i++)
    {
        ItemType x;
        if (fscanf(fi, "%d", &x) != 1) // Kiểm tra lỗi đọc dữ liệu
        {
            printf("Lỗi đọc dữ liệu tại phần tử thứ %d.\n", i);
            break;
        }

        SNode* p = createSNode(x);
        if (p == NULL) // Kiểm tra lỗi cấp phát bộ nhớ
        {
            printf("Lỗi cấp phát bộ nhớ cho node.\n");
            break;
        }

        // Chèn vào danh sách liên kết
        if (insertTail(sl, p) != 1)
        {
            printf("Lỗi chèn node vào danh sách.\n");
            break;
        }
    }
